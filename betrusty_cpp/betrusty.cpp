...
void BeTrusty_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void BeTrusty_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 51;}
...
ctx->componentMemory[coffset].templateName = "BeTrusty";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_add(&expaux[1],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 4]); // line circom 19
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 5]); // line circom 19
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[2]); // line circom 28
...
BeTrusty_3_create(1,0,ctx,"main",0);
BeTrusty_3_run(0,ctx);
}
